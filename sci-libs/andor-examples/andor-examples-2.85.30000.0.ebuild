...
# TODO: RDEPEND on dev-python/ctypes
# $ export LD_LIBRARY_PATH=/path/to/symlink/libandor.so
# $ python
# >>> from ctypes import *
# >>> cdll.LoadLibrary('libandor.so')
# <CDLL 'libandor.so', handle fe7806e0 at 80dfcec>
# >>> libc = CDLL('libandor.so')
# >>> libc.Initialize("/usr/local/etc/andor")
#
# TODO: Dictionary of Error codes
# echo "err = {" > ~/error_codes.py; cat /usr/include/atmcdLXd.h |
#   sed -e '/ 20[0-9][0-9][0-9]/ !d' -e 's/#define //' -e 's|\(.*\) \([0-9]*\)|\2:"\1",|g' >>
#   ~/error_codes.py; sed -i '$ s/,//' ~/error_codes.py; echo "}" >> ~/error_codes.py
#
# TODO: function wrapping
# cat /usr/include/atmcdLXd.h | 
#   sed -e '/^unsigned int/ !d' -e 's/unsigned int //' -e 's/;//' -e "s|\(.*\)(\(.*)\)|\1 = cfunc('\1',\
#\2|"
