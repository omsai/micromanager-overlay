...
# - Test libandor with modern tarballs.  Make one for 2.90 and 2.91
# - Make andor (3) slotted ebuild
...
src_install() {
	...
	insinto /etc/andor
	newins andor/etc/DetectorTemplate.ini Detector.ini \
              || die "newins PCI configuration file failed"

	# USB udev rule
	insinto /etc/udev/rules.d
	newins andor/script/andor.rules andor-usb.rules \
               || die "newins usb udev rule failed"

	# Documentation
	insinto ${ANDOR_HOME}/doc
	doins andor/doc/*.pdf || die "doins *.pdf failed"
	dodoc andor/INSTALL || die "doins INSTALL failed"
	dodoc andor/ReleaseNotes || die "doins ReleaseNotes failed"
}
